...
# Install senpi trading-recipe into /opt/senpi-extensions (not OpenClaw's bundled path) so
...
RUN cd /tmp \
  && echo '{"name":"plugin-install","dependencies":{"@senpi/trading-recipe":"latest"}}' > package.json \
  && npm install --omit=dev \
  && mkdir -p /opt/senpi-extensions \
  && cp -r node_modules/@senpi/trading-recipe /opt/senpi-extensions/trading-recipe \
  && cd /opt/senpi-extensions/trading-recipe && npm install --omit=dev \
